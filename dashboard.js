...
async function loadAllData() {
    try {
        await Promise.all([
            ...
            loadAccounts(),
            loadGoals(),
            loadCreditCards() // Adicionar carregamento de cartões de crédito
        ]);
...
async function updateAllDisplays() {
    ...
    displayAccounts();
    displayGoals();
    displayCreditCards(); // Adicionar display dos cartões de crédito
...
async function loadCreditCards() {
    try {
        const data = await apiCall('/api/credit-cards');
...
async function loadCreditCardsPage() {
    // Carregar cartões se não estiverem carregados
    if (creditCards.length === 0) {
        await loadCreditCards();
    }
    
    // Atualizar estatísticas e display
    updateCreditCardsQuickStats();
    displayCreditCards();
}
...
function displayCreditCards() {
    ...
    if (creditCards.length === 0) {
        container.innerHTML = `
            <div class="empty-state">
                ...
                <p>Configure seus cartões para ter controle automático de reset.</p>
                <button onclick="openCreditCardModal()" class="btn-primary">Adicionar Primeiro Cartão</button>
            </div>
        `;
        return;
    }
    ...
    container.innerHTML = creditCards.map(card => {
        const utilizationPercent = card.credit_limit > 0 ? ((card.current_balance / card.credit_limit) * 100).toFixed(1) : 0;
        const availableCredit = card.credit_limit - card.current_balance;
        const daysUntilReset = calculateDaysUntilReset(card.bill_closing_day);
        const isNearLimit = utilizationPercent > 80;
        const isOverLimit = card.current_balance > card.credit_limit;
        
        return `
            <div class="credit-card-item ${isOverLimit ? 'over-limit' : isNearLimit ? 'near-limit' : ''}" data-card-id="${card._id}">
                <div class="credit-card-header">
                    <div class="credit-card-info">
                        <h3>${card.name}</h3>
                        <p class="card-number">${card.card_number || '**** **** **** 1234'}</p>
                    </div>
                    <div class="credit-card-status">
                        ${card.auto_reset ? '<span class="status-badge auto">Auto Reset</span>' : '<span class="status-badge manual">Manual</span>'}
                    </div>
                </div>
                ...
                <div class="credit-card-balance">
                    <div class="balance-item">
                        ...
                        <span class="balance-value ${isOverLimit ? 'negative' : ''}">R$ ${formatCurrency(card.current_balance)}</span>
                    </div>
                    <div class="balance-item">
                        <span class="balance-label">Limite:</span>
                        <span class="balance-value">R$ ${formatCurrency(card.credit_limit)}</span>
                    </div>
                    <div class="balance-item">
                        <span class="balance-label">Disponível:</span>
                        <span class="balance-value ${availableCredit < 0 ? 'negative' : ''}">R$ ${formatCurrency(availableCredit)}</span>
                    </div>
                </div>
                
                <div class="credit-card-utilization">
                    <div class="utilization-bar">
                        <div class="utilization-fill ${isOverLimit ? 'over' : isNearLimit ? 'near' : ''}" 
                             style="width: ${Math.min(utilizationPercent, 100)}%"></div>
                    </div>
                    <div class="utilization-text">
                        <span>Utilização: ${utilizationPercent}%</span>
                        ${isOverLimit ? '<span class="alert">⚠️ Acima do limite!</span>' : 
                          isNearLimit ? '<span class="warning">⚡ Perto do limite!</span>' : ''}
                    </div>
                </div>
                
                <div class="credit-card-schedule">
                    <div class="schedule-item">
                        <span class="schedule-label">Fechamento:</span>
                        <span class="schedule-value">${card.bill_closing_day}º dia</span>
                    </div>
                    <div class="schedule-item">
                        <span class="schedule-label">Próximo Reset:</span>
                        <span class="schedule-value">${daysUntilReset <= 7 ? '🔴' : daysUntilReset <= 15 ? '🟡' : '🟢'} ${daysUntilReset} dias</span>
                    </div>
                </div>
                
                <div class="credit-card-actions">
                    <button onclick="editCreditCard('${card._id}')" class="btn-secondary" style="padding: 0.5rem 1rem; font-size: 0.875rem;">
                        ⚙️ Configurar
                    </button>
                    <button onclick="manualResetCreditCard('${card._id}')" class="btn-warning" style="padding: 0.5rem 1rem; font-size: 0.875rem;">
                        🔄 Reset Manual
                    </button>
                    <button onclick="viewCreditCardHistory('${card._id}')" class="btn-secondary" style="padding: 0.5rem 1rem; font-size: 0.875rem;">
                        📋 Histórico
                    </button>
                    <button onclick="deleteCreditCard('${card._id}')" class="btn-danger">
                        🗑️ Excluir
                    </button>
                </div>
            </div>
...
function updateCreditCardsQuickStats() {
    const statsContainer = document.getElementById('creditCardsQuickStats');
    ...
    const averageUtilization = totalLimit > 0 ? ((totalBalance / totalLimit) * 100).toFixed(1) : 0;
    const upcomingResets = creditCards.filter(card => {
        const days = calculateDaysUntilReset(card.bill_closing_day);
        return days <= 7;
    }).length;
    
    document.getElementById('totalCreditCards').textContent = totalCards;
    document.getElementById('totalCreditLimit').textContent = `R$ ${formatCurrency(totalLimit)}`;
    document.getElementById('averageUtilization').textContent = `${averageUtilization}%`;
    document.getElementById('upcomingResets').textContent = upcomingResets;
}

function calculateDaysUntilReset(closingDay) {
    ...
    if (currentDay <= closingDay) {
        // Próximo reset é neste mês
        nextResetDate = new Date(currentYear, currentMonth, closingDay);
    } else {
        // Próximo reset é no próximo mês
        nextResetDate = new Date(currentYear, currentMonth + 1, closingDay);
    }
...
function openCreditCardModal(card = null) {
    ...
    if (card) {
        document.getElementById('creditCardModalTitle').textContent = 'Editar Cartão de Crédito';
        document.getElementById('creditCardId').value = card._id;
        document.getElementById('creditCardName').value = card.name;
        document.getElementById('creditCardNumber').value = card.card_number || '';
        document.getElementById('creditCardLimit').value = card.credit_limit;
        document.getElementById('creditCardCurrentBalance').value = card.current_balance;
        document.getElementById('creditCardBillClosingDay').value = card.bill_closing_day;
        document.getElementById('creditCardAutoReset').checked = card.auto_reset;
    } else {
        document.getElementById('creditCardModalTitle').textContent = '💳 Novo Cartão de Crédito';
    }
...
async function saveCreditCard(event) {
    ...
    const cardId = document.getElementById('creditCardId').value;
    const cardData = {
        name: document.getElementById('creditCardName').value,
        card_number: document.getElementById('creditCardNumber').value || null,
        credit_limit: parseFloat(document.getElementById('creditCardLimit').value),
        current_balance: parseFloat(document.getElementById('creditCardCurrentBalance').value),
        bill_closing_day: parseInt(document.getElementById('creditCardBillClosingDay').value),
        auto_reset: document.getElementById('creditCardAutoReset').checked
    };
    ...
    try {
        ...
        showNotification(error.message || 'Erro ao salvar cartão', 'error');
    }
}
...
async function manualResetCreditCard(cardId) {
    if (!confirm('Tem certeza que deseja executar o reset manual deste cartão?')) {
        return;
    }
    
    try {
        await apiCall(`/api/credit-cards/${cardId}/reset`, {
            method: 'POST'
        });
        
        showNotification('Reset manual executado com sucesso!', 'success');
        await loadCreditCards();
        updateCreditCardsQuickStats();
        displayCreditCards();
    } catch (error) {
        console.error('Erro ao executar reset manual:', error);
        showNotification(error.message || 'Erro ao executar reset manual', 'error');
    }
}
...
async function viewCreditCardHistory(cardId) {
    try {
        const data = await apiCall(`/api/credit-cards/${cardId}/history`);
        const history = data.history || [];
        
        const modal = document.getElementById('creditCardHistoryModal');
        const historyList = document.getElementById('creditCardHistoryList');
        
        const card = creditCards.find(c => c._id === cardId);
        document.getElementById('historyModalTitle').textContent = `📋 Histórico - ${card?.name || 'Cartão'}`;
        ...
        if (history.length === 0) {
            ...
            historyList.innerHTML = history.map(entry => `
                <div class="history-item">
                    <div class="history-date">${formatDate(entry.reset_date)}</div>
                    <div class="history-action">${entry.action}</div>
                    <div class="history-details">
                        Saldo anterior: R$ ${formatCurrency(entry.previous_balance)}<br>
                        Novo saldo: R$ ${formatCurrency(entry.new_balance)}
                    </div>
                </div>
            `).join('');
        }
        ...
        showNotification('Erro ao carregar histórico do cartão', 'error');
    }
}

function closeCreditCardHistoryModal() {
    document.getElementById('creditCardHistoryModal').style.display = 'none';
}

// Initialize Credit Card Manager
function initializeCreditCardManager() {
    // O creditCardManager será definido globalmente para acesso por onclick handlers
    window.creditCardManager = {
        async getUpcomingResets() {
            try {
                const data = await apiCall('/api/credit-cards/upcoming-resets');
                const upcoming = data.upcoming_resets || [];
                
                const modal = document.getElementById('upcomingResetsModal');
                const list = document.getElementById('upcomingResetsList');
                
                if (upcoming.length === 0) {
                    list.innerHTML = '<p style="text-align: center; color: var(--text-secondary);">Nenhum reset programado</p>';
                } else {
                    list.innerHTML = upcoming.map(item => `
                        <div class="upcoming-reset-item">
                            <div class="reset-card-info">
                                <h4>${item.card_name}</h4>
                                <p>Limite: R$ ${formatCurrency(item.credit_limit)}</p>
                            </div>
                            <div class="reset-date">
                                <strong>${formatDate(item.next_reset_date)}</strong>
                                <small>${item.days_until_reset} dias</small>
                            </div>
                        </div>
                    `).join('');
                }
                ...
                showNotification('Erro ao carregar próximos resets', 'error');
            }
        },
        ...
        async getStats() {
            try {
                ...
                const autoResetCards = creditCards.filter(card => card.auto_reset).length;
                
                showNotification(
                    `📊 Estatísticas:\n\n` +
                    `Cartões: ${totalCards}\n` +
                    `Limite Total: R$ ${formatCurrency(totalLimit)}\n` +
                    `Saldo Total: R$ ${formatCurrency(totalBalance)}\n` +
                    `Utilização: ${utilization}%\n` +
                    `Auto Reset: ${autoResetCards} cartões`,
                    'info',
                    8000
                );
            } catch (error) {
...
function closeUpcomingResetsModal() {
    document.getElementById('upcomingResetsModal').style.display = 'none';
}
...
// [Continue with all the original functions from the dashboard.js file]
// For brevity, I'm including the essential ones and the credit card integration
...
// Additional helper functions for credit cards
...
// [Include all other original functions from dashboard.js]
// For brevity, I'm focusing on the credit card integration
...
function updateCategoryPieChart() {
    ...
    charts.categoryPie.data.datasets[0].backgroundColor = [
